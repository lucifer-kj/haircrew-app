          ...
          <div className="flex items-center justify-center mb-8">
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col items-center">
                ...
                <span className="text-xs mt-1 font-medium text-gray-400">Shipping Method</span>
              </div>
              <div className="w-8 h-0.5 bg-gray-300" />
              <div className="flex flex-col items-center">
                <div className="w-8 h-8 rounded-full bg-gray-200 text-gray-500 flex items-center justify-center font-bold">3</div>
                <span className="text-xs mt-1 font-medium text-gray-400">Payment Method</span>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
                ...
                >
                  ...
                  {addresses.map(addr => (
                    <option key={addr.id} value={addr.id}>
                      {addr.name}, {addr.address}, {addr.city}, {addr.state}, {addr.pincode}, {addr.country}
                    </option>
                  ))}
                </select>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Input name="name" value={form.name} onChange={handleChange} placeholder="Full Name" required aria-invalid={!!error && !form.name} />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Phone Number</label>
                <Input name="phone" value={form.phone} onChange={handleChange} placeholder="Phone Number" required aria-invalid={!!error && !form.phone} />
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Address</label>
              <Input name="address" value={form.address} onChange={handleChange} placeholder="Address" required aria-invalid={!!error && !form.address} />
              {(!form.address && error) && <div className="text-red-500 text-xs mt-1">This value is required</div>}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium mb-1">City</label>
                <Input name="city" value={form.city} onChange={handleChange} placeholder="City" required aria-invalid={!!error && !form.city} />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Pincode</label>
                <Input name="pincode" value={form.pincode} onChange={handleChange} placeholder="Pincode" required aria-invalid={!!error && !form.pincode} />
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
            ...
            <Button type="submit" className="w-full bg-orange-500 hover:bg-orange-600 text-white text-base font-semibold py-3 rounded-md shadow-md mt-4">Continue to Review</Button>
          </form>
        </div>
        {/* Right: Order Summary */}
