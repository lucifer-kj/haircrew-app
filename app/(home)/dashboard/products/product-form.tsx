  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <input type="number" value={stock} onChange={e => setStock(e.target.value)} className="border rounded px-2 py-1 w-full" required />
      </div>
      <div>
        <label className="block text-sm font-medium mb-1">Category ID</label>
        <input type="text" value={categoryId} onChange={e => setCategoryId(e.target.value)} className="border rounded px-2 py-1 w-full" required />
      </div>
      <div>
        ...
        <div className="flex gap-2 mt-2">
          {images.map(url => (
            <Image key={url} src={url} alt="Product" width={64} height={64} className="object-cover rounded" />
          ))}
        </div>
      </div>
      <div className="flex gap-2 justify-end">
        <button type="button" className="px-4 py-2 rounded bg-slate-500 text-white" onClick={onClose}>Cancel</button>
