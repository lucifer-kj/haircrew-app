...
const BRAND_LOGO_URL = `${process.env.NEXT_PUBLIC_BASE_URL || ''}/favicon.ico`;
const BRAND_PRIMARY = '#9929EA';
const BRAND_SECONDARY = '#891997';
const BRAND_FONT = 'Inter, -apple-system, BlinkMacSystemFont, Segoe UI, Roboto, sans-serif';
...
export function orderConfirmationTemplate({ name, orderId }: { name: string; orderId: string }) {
  return emailBase({
    body: `
      ...
      <a href="${process.env.NEXT_PUBLIC_BASE_URL || ''}/order-received/${orderId}" style="display:inline-block;margin-top:24px;padding:12px 32px;background:${BRAND_PRIMARY};color:#fff;border-radius:8px;text-decoration:none;font-weight:600;">View Order</a>
    `
  });
}

export function shippingUpdateTemplate({ name, orderId, status }: { name: string; orderId: string; status: string }) {
  return emailBase({
    body: `
      ...
      <a href="${process.env.NEXT_PUBLIC_BASE_URL || ''}/order-received/${orderId}" style="display:inline-block;margin-top:24px;padding:12px 32px;background:${BRAND_PRIMARY};color:#fff;border-radius:8px;text-decoration:none;font-weight:600;">Track Order</a>
    `
  });
}
